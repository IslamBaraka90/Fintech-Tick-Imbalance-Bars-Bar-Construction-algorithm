 ...
 * pre-aggregated bars, where the signs are already lost — so there is no live
 * Yahoo source for this algorithm.
 */
...
export function loadTrades(csvPath: string): Trade[] {
  ...
  return lines.slice(1).map((line) => {
    ...
    if (row.sequence !== undefined && row.sequence !== "") trade.sequence = Number(row.sequence);
